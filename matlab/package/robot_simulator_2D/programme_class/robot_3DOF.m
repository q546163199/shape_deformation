classdef robot_3DOF
    ...
    methods
        function obj = robot_3DOF(link1,link2,link3,biasx,biasy,elbow_case)
            %UNTITLED Construct an instance of this class
            %   Detailed explanation goes here
            obj.link1_length = link1;
            obj.link2_length = link2;
            obj.link3_length = link3;
            obj.biasx = biasx;
            obj.biasy = biasy;
            obj.elbow_case = elbow_case;
        end
    
        function [p1,p2,p3] = fkine(obj,q)
           %% initial parameter
            l1 = obj.link1_length;
            l2 = obj.link2_length;
            l3 = obj.link3_length;
           ...
           %%
            ...
            p3.x = l1 * cos(q1) + l2 * cos(q1 + q2) + l3 * cos(q1 + q2 + q3) + bias.x;
            p3.y = l1 * sin(q1) + l2 * sin(q1 + q2) + l3 * sin(q1 + q2 + q3) + bias.y;
        end
        ...
        function q = ikine(obj,p3,pitch)
           %% initial parameter
            ...
            bias.y = obj.biasy;
           %%
            p2.x = p3.x - l3 * cos(pitch);
            p2.y = p3.y - l3 * sin(pitch);
            x = p2.x - bias.x;
            y = p2.y - bias.y;
        ...
        function handle = plot(obj,q)
           %% initial parameter
            ...
            bias.y = obj.biasy;
           %%
           [p1,p2,p3] = obj.fkine(q);
           %% plot figure
           handle = plot([bias.x p1.x p2.x p3.x],[bias.y p1.y p2.y p3.y],'k-*','linewidth',2);
           %% axis limitation
           axis_limit = l1 + l2 + l3 + max(bias.x,bias.y)/2;
