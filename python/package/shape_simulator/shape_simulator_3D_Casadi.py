...
def CalcPosition(s, param):
    ...
    for i in range(int((s-s0)/ds) + 1):
        sc = ds * i
        if Re != 0.0:
            epsilon_l = f_evaluate(sc, a_epsilon, 0.0, e)

        theta_l = f_evaluate(sc, a_theta, state0[4], e)
        phi_l = f_evaluate(sc, a_phi, state0[3], e)
...
def CalcOrientation(s, param):
    ...
    phi = f_evaluate(s, a_phi, state0[3], e)
    theta = f_evaluate(s, a_theta, state0[4], e)
    psi = f_evaluate(s, a_psi, state0[5], e)
...
def costfun(param, e):
    ...
    for i in range(N + 1):
        ...
        if Re is not 0.0:
            a_epsilon = param[3 * n + 0:4 * n, 0]
            epsilon = f_evaluate(s, a_epsilon, 0.0, e)
        ...
        if D is not 0.0:
            ...
            for j in range(int((s-s0)/ds)+1):
                sl = ds * j
                if Re is not 0.0:
                    epsilon_l = f_evaluate(sl, a_epsilon, 0.0, e)

                theta_l = f_evaluate(sl, a_theta, state0[4], e)
                phi_l = f_evaluate(sl, a_phi, state0[3], e)
...
def constraint_eq(param, ceq, e):
    ...
    # Constraints at s = s0
    i = 0
    ceq[i] = state0[3] - f_evaluate(s0, a_phi, state0[3], e)
    i = i + 1
    ceq[i] = state0[4] - f_evaluate(s0, a_theta, state0[4], e)
    i = i + 1
    ceq[i] = state0[5] - f_evaluate(s0, a_psi, state0[5], e)
    if Re != 0.0:
        i = i + 1
        ceq[i] = f_evaluate(s0, a_epsilon, 0.0, e)

    # Constraints at s = s1
    # Orientations
    i = i + 1
    ceq[i] = state1[3] - f_evaluate(s1, a_phi, state0[3], e)
    i = i + 1
    ceq[i] = state1[4] - f_evaluate(s1, a_theta, state0[4], e)
    i = i + 1
    ceq[i] = state1[5] - f_evaluate(s1, a_psi, state0[5], e)
